...
def process_file(file_path: Path, config: Dict, file_num: int, total_files: int) -> bool:
    ...
    try:
        ...
        if file_path.suffix == '.mp3':
            print(f"{Colors.GRAY}  ✓ Already MP3, skipping conversion{Colors.RESET}")
...
def main():
    ...
    parser.add_argument('-i', '--input', help='Input folder containing audio/video files')
    parser.add_argument('-o', '--output', help='Output folder for transcripts')
    parser.add_argument('-m', '--model', help='Whisper model (e.g., large-v2, medium, small)')
    parser.add_argument('-d', '--device', choices=['cuda', 'cpu'], help='Processing device')
    parser.add_argument('--max-duration', type=int, help='Max duration before split (seconds)')
